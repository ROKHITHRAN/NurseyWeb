      ...
      <div className="relative">
        ...
        <div className="p-4">
          <Link to={`/plant/${plant.id}`}>
            <h3 className="text-lg font-semibold text-gray-800 hover:text-green-600 mb-1">{plant.title}</h3>
          </Link>
          ...
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-baseline">
              {isDiscounted ? (
                <>
                  <span className="text-lg font-bold text-green-700">${plant.discountedPrice?.toFixed(2)}</span>
                  <span className="text-sm text-gray-500 line-through ml-2">${plant.price.toFixed(2)}</span>
                </>
              ) : (
                <span className="text-lg font-bold text-green-700">${plant.price.toFixed(2)}</span>
              )}
            </div>
            <span className={`text-sm ${plant.stock > 10 ? 'text-green-600' : plant.stock > 0 ? 'text-orange-500' : 'text-red-500'}`}>
